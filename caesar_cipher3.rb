...
def caesar_cipher(offset, string)
  ...
  while word_idx < words.length
    word = words[word_idx]
    puts("word[" + word_idx.to_s + "] = " + word)
    letter_idx = 0
    while letter_idx < word.length
      char_i = word[letter_idx].ord - "a".ord
      puts("char_i = " + char_i.to_s)
      new_char_i = (char_i + offset) % 26
      puts("new_char_i = " + new_char_i.to_s)
      word[letter_idx] = ("a".ord + new_char_i).chr
      puts("word[" + letter_idx.to_s + "] = " + word[letter_idx] + "a".ord.to_s + " + new_char_i " + new_char_i.to_s)
      puts nil
